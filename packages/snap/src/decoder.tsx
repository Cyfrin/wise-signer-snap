...
import { Transaction, FourByteResponse, DecodedResult } from './types';

// Extract function signature from 4byte.directory response and try to decode with each one
async function getFunctionSignatureAndDecode(selector: string, encodedParams: string): Promise<{ signature: string; inputs: string[]; decodedInputs: any[] } | null> {
...
function parseFunctionSignature(signature: string): { name: string; inputs: string[] } {
    ...
    if (!name) {
        throw new Error(`Function name could not be parsed from signature: ${signature}`);
    }
    const inputs = paramsStr ? paramsStr.split(',').map(param => param.trim()) : [];

    return { name, inputs };
}

// Recursively decode any bytes parameters that might contain nested function calls
async function decodeNestedBytes(decodedInputs: any[], inputTypes: string[]): Promise<DecodedResult[]> {
    ...
    for (let i = 0; i < decodedInputs.length; i++) {
        ...
        if (inputType === 'bytes' && typeof input === 'string' && input.length >= 10) {
...
export async function decode(transaction: Transaction): Promise<DecodedResult> {
    ...
    const decodingResult = await getFunctionSignatureAndDecode(selector, encodedParams);
